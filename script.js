...
class ContactForm {
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            this.showStatus('error', 'Failed to send message. Please try again.');
            Analytics.trackEvent('contact_form_submit', { success: false, error: error.message });
        }
    }
...
class FreelanceForm {
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            if (response.ok) {
                this.showStatus('success', 'Inquiry submitted successfully! I\'ll review and get back to you within 48 hours.');
                this.form.reset();
                Analytics.trackEvent('freelance_inquiry_submit', { success: true, budget: data.budget });
            } else {
            ...
            this.showStatus('error', 'Failed to submit inquiry. Please try again.');
            Analytics.trackEvent('freelance_inquiry_submit', { success: false, error: error.message });
        }
    }
...
class ScrollAnimations {
    constructor() {
        this.elements = document.querySelectorAll('.timeline-item, .project-card, .skill-category');
        this.init();
    }
